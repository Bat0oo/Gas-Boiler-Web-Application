...
const AlarmsPage: React.FC = () => {
  ...
  const handleAcknowledge = async (id: number) => {
    ...
    try {
      ...
      alert(
        `Error: ${err.response?.data?.message || "Failed to acknowledge alarm"}`,
      );
    }
  };
  ...
  const handleResolve = async (id: number) => {
    ...
    try {
      ...
      alert(
        `Error: ${err.response?.data?.message || "Failed to resolve alarm"}`,
      );
    }
  };
  ...
  const handleDelete = async (id: number) => {
    ...
    try {
      ...
      alert(
        `Error: ${err.response?.data?.message || "Failed to delete alarm"}`,
      );
    }
  };
  ...
  const getAlarmTypeLabel = (type: string) => {
    switch (type) {
      ...
      case "HIGH_INDOOR_TEMP":
        return "High Indoor Temp";
      case "LOW_INDOOR_TEMP":
        return "Low Indoor Temp";
      case "HIGH_OUTDOOR_TEMP":
        return "High Outdoor Temp";
      case "LOW_OUTDOOR_TEMP":
        return "Low Outdoor Temp";
      case "HIGH_DAILY_COST":
        return "High Cost";
      default:
        return type;
    }
            ...
            <label>
              <input
                ...
                onChange={(e) =>
                  setFilters({
                    ...filters,
                    isAcknowledged: e.target.checked ? false : undefined,
                  })
                }
              />
              Unacknowledged only
              ...
              >
                <div className="alarm-card-header">
                  <div
                    ...
                    style={{ background: getSeverityColor(alarm.severity) }}
                  >
                    {getSeverityIcon(alarm.severity)} {alarm.severity}
                  </div>
                  <div className="alarm-type-badge">
