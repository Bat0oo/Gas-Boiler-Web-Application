...
export const useRealtimeBoilers = (token: string) => {
  const [boilerPowers, setBoilerPowers] = useState<
    Map<number, BoilerPowerUpdate>
  >(new Map());
  const [temperatures, setTemperatures] = useState<
    Map<number, IndoorTempUpdate>
  >(new Map());
  const [capacityWarnings, setCapacityWarnings] = useState<
    Map<number, CapacityWarning>
  >(new Map());
  ...
  useSignalREvent<BoilerPowerUpdate>(
    connection,
    "BoilerPowerUpdated",
    (data) => {
  ...
  useSignalREvent<IndoorTempUpdate>(
    connection,
...
export const useBuildingRealtime = (buildingId: number, token: string) => {
  ...
  const buildingBoilers = Array.from(boilerPowers.values()).filter(
