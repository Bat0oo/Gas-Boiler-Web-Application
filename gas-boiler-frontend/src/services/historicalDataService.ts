...
import { BuildingReading, SeedHistoricalDataResponse } from '../types/historicaldatatypes';
...
export const historicalDataService = {
  ...
  async getReadingsCount(token: string): Promise<number> {
    const response = await apiClient.get<{ totalReadings: number }>('/HistoricalData/count', {
      headers: { Authorization: `Bearer ${token}` },
    });
